...
module.exports = {
    validateRegister: async (req,res,next) => {
        ...
        if (req.body.username.length <4 || req.body.username.length >20) return resSend(res, true, null, 'Username should be between 4 and 20 characters long.')
        if(!req.body.pass1) return resSend(res, true, null, 'Password cannot be empty');
        if (req.body.pass1.length <4 || req.body.pass1.length >20) return resSend(res, true, null, 'Password should be between 4 and 20 characters long.')
        if(!passRegex.test(req.body.pass1)) return resSend(res, true, null, 'Password should have at least one upper case letter.')
    ...
    validateLogin: async (req,res, next) => {
        ...
        if(!findUser) return resSend(res, true, null, 'User not found.');
    ...
    validatePassChange: async (req,res,next) => {
        ...
        if(!oldPass) return resSend(res, true, null, 'Password cannot be empty');
        if(!newPass) return resSend(res, true, null, 'Password cannot be empty');
        if (newPass.length <4 || newPass.length >20) return resSend(res, true, null, 'Password should be between 4 and 20 characters long.')
        if(!passRegex.test(newPass)) return resSend(res, true, null, 'Password should have at least one upper case letter.')
        if(newPass !== repeatNewPass) return resSend(res, true, null, 'Passwords should match.');
        const findUser = await userDb.findOne({username});
        if(!findUser) return resSend(res, true, null, 'User not found.');
